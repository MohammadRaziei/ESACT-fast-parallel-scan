...
function Text(x, y, width, height, mathExpression, color="black") {
  this.x = x;
  this.y = y;
...
const globalMemoryColor = "#BE80F9"; //"#A855F7";
const sharedMemoryColor = "#6CA1F8"; //#3B82F6";
const sumNodeColor = "#9591F9";
...
class Node {
  ...
  constructor(x, y, colorfill = sharedMemoryColor, text = "") {
    this.x = x;
    this.y = y;
...
function create_svg(number_nodes) {
  ...
  var input_nodes = Array.from({ length: number_nodes }, (_, i) =>
    new Node(i * Node.width, 0, globalMemoryColor, `X_{${i}}`).draw()
  );
    ...
    case "blelloch":
      {
        ...
        sum_node.elem.attr("fill", sumNodeColor);
        if (number_nodes > 1)
